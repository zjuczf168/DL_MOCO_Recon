...
def center_crop_pad(data: torch.Tensor, shape: Tuple[int, int]) -> torch.Tensor:
    """
    Apply a center crop/pad to the input real image or batch of real images.
    ...
    if w >= shape[0]:
        ...
        pad_w = (shape[0]-w) // 2
        data = F.pad(data, (0, 0, pad_w, pad_w), "constant", 0)
    ...
    if h >= shape[1]:
        ...
        pad_h = (shape[1]-h) // 2
        data = F.pad(data, (pad_h, pad_h), "constant", 0)
