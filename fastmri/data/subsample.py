...
class RandomMaskFunc_bkp(MaskFunc):
    """
...
class RandomMaskFunc(MaskFunc):
    ...
    def __call__(
        ...
        with temp_seed(self.rng, seed):
            ...
            # --- create the mask center --- #
            mask = np.zeros(num_cols)
            num_low_freqs = int(round(num_cols * center_fraction))
            pad = (num_cols - num_low_freqs + 1) // 2
            mask[pad : pad + num_low_freqs] = True
            # --- create the mask random --- #
            x = np.concatenate((np.arange(pad), pad + num_low_freqs+np.arange(num_cols-pad-num_low_freqs)))
            self.rng.shuffle(x)
            num_rand_sample = int(num_cols/float(acceleration) - num_low_freqs)
            mask[x[0:num_rand_sample]] = True
...
def create_mask_for_mask_type(
    ...
    elif mask_type_str == "lowres":
        return LowResMaskFunc(center_fractions, accelerations)
    else:
